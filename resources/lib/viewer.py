...
class Viewer:
    ...
    def __init__(self, *args, **kwargs):
        ...
        msg = {
            ...
            "license": 30763,
            "properties": 30764
        }[self.m_addon.params["view"]]
        # give window time to initialize
        xbmc.sleep(100)
        # set heading
        window.getControl(self.CONTROL_LABEL).setLabel("{msg} - {name}".format(
            msg=self.m_addon.getLocalizedString(msg + 5),
        ...
        # fetch correct info
        if (self.m_addon.params["view"] in ["updates", "changelog"]):
            text = self._fetch_changelog(self.m_addon.params["view"], msg=msg + 5)
        elif (self.m_addon.params["view"] in ["readme", "license"]):
            text = self._fetch_text_file(self.m_addon.params["view"], msg=msg + 5)
        elif (self.m_addon.params["view"] == "properties"):
    ...
    def _fetch_changelog(self, view, msg):
        ...
        date_format = "{date} {time}".format(
            date=xbmc.getRegion("datelong"),
            time=xbmc.getRegion("time")
        )
        # get client
        client = pysvn.Client()
        ...
        try:
            ...
            # changelog
            log = client.log(
                url_or_path="{repo}{branch}{id}".format(
                    repo=self.m_addon.getSetting("Repo"),
                    branch=self.m_addon.getSetting("Branch"),
